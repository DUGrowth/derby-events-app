...
export class DerbyCouncilScraper extends BaseScraper {
  private baseUrl = 'https://www.derby.gov.uk/events/';

  constructor() {
    super('Derby City Council');
  }
  ...
  async fetchEvents(): Promise<ScrapedEvent[]> {
    ...
    const candidates = $('article, .event, .listing, .search-result').toArray();
